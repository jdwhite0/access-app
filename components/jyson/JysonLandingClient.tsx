...
export default function JysonLandingClient({ isSignedIn }: Props) {
  ...
  function handleSignedInAction(href: string) {
    if (!isSignedIn) {
      setShowAuthPrompt(true)
      return
    }
  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            ...
            <span style={{ fontSize: 10, color: '#697386', marginLeft: 6, letterSpacing: '0.04em' }}>by JD AI Systems</span>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {isSignedIn ? (
            <>
              <Link href="/companion" style={{ fontSize: 13, color: '#425466', textDecoration: 'none', padding: '6px 10px', borderRadius: 6 }}>Open JYSON</Link>
            ...
            <>
              <Link href="/sign-in" style={{ fontSize: 13, color: '#425466', textDecoration: 'none', padding: '6px 10px' }}>Log in</Link>
              <Link href="/sign-up" style={{ fontSize: 13, fontWeight: 600, color: '#fff', textDecoration: 'none', background: '#0a2540', padding: '7px 16px', borderRadius: 7 }}>Sign up</Link>
            </>
          )}
        </div>
      </header>
      ...
      <main style={{
        ...
        flexDirection: 'column',
        alignItems: 'center',
        padding: 'clamp(80px, 14vw, 120px) clamp(16px, 5vw, 40px) 120px',
        maxWidth: 720,
        margin: '0 auto',
        width: '100%',
      }}>
        ...
        <p style={{ fontSize: 15, color: '#697386', margin: '0 0 32px', textAlign: 'center', lineHeight: 1.5 }}>
          Your AI operator is ready. Tell JYSON what you need.
        </p>
        ...
        <div style={{
          ...
          <textarea
            ...
            onFocus={() => { if (!isSignedIn) setShowAuthPrompt(true) }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingBottom: 6 }}>
            {/* Attach */}
            <button onClick={() => handleSignedInAction('/companion')} style={{ width: 34, height: 34, borderRadius: 8, border: '1px solid #e6ebf1', background: '#fff', color: '#697386', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }} title="Attach">+</button>
            {/* Send */}
            <button
              onClick={handleSend}
              style={{
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginBottom: 32, width: '100%' }}>
          {CHIPS.map((chip) => (
            <button
              key={chip}
              onClick={() => { setInput(chip); if (!isSignedIn) setShowAuthPrompt(true) }}
              style={{
                fontSize: 13, color: '#425466', background: '#fff',
                border: '1px solid #e6ebf1', borderRadius: 100,
        ...
        {/* Auth prompt card — always shown if not signed in */}
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <Link href="/sign-in" style={{
                fontSize: 14, fontWeight: 500, color: '#0a2540',
                textDecoration: 'none',
                background: 'transparent',
      ...
      <footer style={{
        ...
        <p style={{ fontSize: 11, color: '#697386', margin: 0, lineHeight: 1.4 }}>
          By messaging JYSON, you agree to our{' '}
          ...
          {' '}
          <Link href="/" style={{ color: '#697386' }}>JYSON</Link>
          {' '}·{' '}
          <Link href="https://jdwhite.world" style={{ color: '#697386' }}>JD AI Systems</Link>
        </p>
      </footer>
    </div>
  )
